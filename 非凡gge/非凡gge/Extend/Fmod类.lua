...
function FMOD:打开_从内存(循环,地址,长度)
	...
	self.stream = ffi.gc(_fmod.FSOUND_Stream_Open(self.地址,循环 and 0x8002 or 0x8000,0,长度 or 0),_fmod.FSOUND_Stream_Close)
	return self
end
function FMOD:播放(channel)
	--self:置音量(0)
	--print(self:取音量())
	if 全局变量.音量==0 then return 0 end
	self:置音量(全局变量.音量)
...
function FMOD:置音量(v)
	_fmod.FSOUND_SetVolume(self.channel,v)
end
function FMOD:是否播放()
