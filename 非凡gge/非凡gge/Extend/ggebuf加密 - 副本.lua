...
function ggebuf:初始化(v)
	...
	self.psd       = psd or 'ggelua'		--设置密码   //add by sun 2019/05/11
end
...
function ggebuf:添加数据( ... )
	...
	self:打包完成()
	-- print("!!!!!!!!!!!!!!!!!!!!!", self.psd)
	if self.psd then		--如果有密码 加密封包  //add by sun 2019/05/11
		-- print(1, 12)
		local ptr = self.data + 12
		-- print(2)
		local psd = ffi.cast('int8_t*',self.psd)
		-- print(3)
		local len = self.len - 12
		-- print(4)
