...
local _was  = require("script/直读系统/SP")
local jls   = require("Script/系统专用/gge精灵类")
local ceil  = math.ceil

function 动画类:初始化(p,len,文件,文件号)
	self.was = _was(p,len)
...
function 动画类:更新行(当前帧)
	if self.信息组[当前帧] == nil then
		self.信息组[当前帧] = self.was:取纹理(当前帧)
		self.精灵:置纹理(self.信息组[当前帧][1])
	end
end
