...
local __jls = require("Script/系统专用/gge精灵类")
...
function 自适应:加载(根)
	...
	适应组[71] = {[0]=资源:载入('wzife.wd4',"网易WDF动画",0x80E0B578).精灵} -- 梦幻的文字信息栏背景
	适应组[31] = {[0]=资源:载入('wzife.wd1',"网易WDF动画",0x4E5F661E).精灵} -- 梦幻的滑块分割条小   [17] = 自适应.创建(31,1,14,210,2,0),  self.资源组[17]:置颜色(-16777216)
	适应组[32] = {[0]=资源:载入('wzife.wd1',"网易WDF动画",0x2D0F136C).精灵} -- 梦幻的滑块分割条大
	适应组[33] = {[0]=资源:载入('wzife.wdf',"网易WDF动画",0xE2C626FA).精灵} -- UI的按钮背景 加分割条
	-- 按钮组
	...
	--分区--########################################################?自己修改?##########################################
    适应组[29] = {按钮=资源:载入('wzife.wdf',"网易WDF动画",0xD330CE3F)}--大分区
    适应组[30] = {按钮=资源:载入('wzife.wdf',"网易WDF动画",0xBD57A592)}--小分区
	-- * 不可组合按钮组
	...
	-- 开始处理图像
	local 九格组 = {0,2,9,4,5,6,71,33}
	local 横排组 = {10,12,1,3,13,7,28,29,30}
	local 竖排组 = {11,31,32}
	for i=1,#九格组 do
		-- 开始复制区域
		local 大小 = 70
