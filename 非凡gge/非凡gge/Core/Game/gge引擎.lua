...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2019-09-19 02:50:48
--@表 引擎
local 引擎 = {}
local _exit = false
local _base
...
local function __FrameLoop(d,x,y)
	if(更新函数)then
		if __gge.safecall(更新函数,d,x,y) then
		    return true
		end
	end
	if(渲染函数)then
		if __gge.safecall(渲染函数,d,x,y) then
		    return true
		end
	end
	if _新标题 then
...
local function __Message(HWND,UMSG,WPARAM,LPARAM)
	-- if UMSG ~=132 and UMSG ~= 32 and UMSG ~= 512 then
	...
	elseif UMSG == 256 then--WM_KEYDOWN
		--print(UMSG,string.char(WPARAM),LPARAM)
	end
	if UMSG ==260 or UMSG ==261 or UMSG ==262 then --ALT
	    return true
	end
	if _消息函数 then __gge.safecall(_消息函数 )end
...
local __ggeBase = require("__ggebase__")
...
function 引擎.创建(标题,宽度,高度,帧率,鼠标)
	...
	引擎.标题 = 标题 or "梦幻西游"
	引擎.原始标题 = 引擎.标题
	_base:System_SetStateChar(1,引擎.标题)
...
function 引擎.渲染开始(tex)
	if(_isrun)then _base:Graph_BeginScene(tex and tex:取指针() or 0) end
end
--@说明 渲染结束
function 引擎.渲染结束()
...
function 引擎.置变换矩阵(x,y,dx,dy,rot,hscale,vscale)
	...
	-- 	float	tx, ty;		///< 纹理坐标
	-- };
-- function 引擎.渲染图元(类型,图元,数量,纹理,混合,过滤)
-- 	if(_isrun)then _base:Graph_RenderBatch(类型,图元,数量,纹理 or 0,混合 or 0,过滤 or false) end
-- end
-- /// 屏幕截图格式
...
function 引擎.截图(file,格式)
	if(_isrun)then _base:Graph_Snapshot(file,格式 or 3) end
end
...
function 引擎.文件是否存在(v)
	if(_isrun)then return _base:Resource_IsExist(v) end
end
--@参数 文件路径,密码
--@返回 成功返回true
