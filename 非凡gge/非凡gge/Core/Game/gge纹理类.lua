...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2019-09-04 15:29:45
--<<=========================================================================================>>--
...
function ggeobj:初始化(...)
	...
	if(...)then
		...
		local lt = type(arg[1])
		if(lt == "string")then
			if type(arg[2])  == "number" and #arg[1]>256 then--2为指针长度--按luastring指针
				local ptr 	= tonumber(ffi.cast("intptr_t",arg[1]))
				assert(self._obj:Texture_LoadMem(ptr,arg[2],arg[3] or 0), "载入失败.")
			else--2为透明颜色--按文件路径载入
		...
		elseif(lt == "number")then	--按指针载入
			if type(arg[2])  == "number" then--2为指针长度
				self._obj:Texture_LoadMem(arg[1],arg[2],arg[3] or 0)--按图片指针
			else
...
function ggeobj:复制(宽度,高度)
	if(self._isok )then
		local islock = self.lock
		if not self.lock then
		    self:锁定(true)
		end
		local p = self._obj:Copy(宽度 or self:取宽度(),高度 or self:取高度())
...
function ggeobj:灰度级()
	local islock = self.lock
	if not self.lock then self:锁定(true) end
	self._obj:Grayscale()
	if not islock then self:解锁() end
	return self
end
