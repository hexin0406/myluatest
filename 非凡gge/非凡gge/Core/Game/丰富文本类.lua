...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2017-05-04 07:49:27
local 丰富文本类 = class()
	...
	if (self.行数量 > 0) then
		...
		for i=self.起始值,self.目标值,self.递增值 do
			if (self.显示表[i]) then
				...
				for i,v in ipairs(行数据) do
					local 显示x		= 行数据.x+self.x+v.x
					local 显示y0 	= math.floor(起始y + self.y + (行数据.高度 - v.高度)/2)
					if v.回调 then
						self.回调包围盒:置坐标宽高(显示x ,显示y0,v.宽度,v.高度)
						if self.回调包围盒:检查点(x,y) then
							if v.回调 == '' or not v.回调 then
								return v.内容
							else
								return v.回调
							end
