...
function 道具处理类:数据处理(连接id,序号,数字id,数据)
  ...
  elseif 序号==3762 then
    if 玩家数据[数字id].角色.数据.修炼[玩家数据[数字id].角色.数据.修炼.当前][1]>=玩家数据[数字id].角色.数据.修炼[玩家数据[数字id].角色.数据.修炼.当前][3] then
      常规提示(数字id,"#Y/你的此项修炼已经达上限")
      return
    end
    if 玩家数据[数字id].角色.数据.当前经验<100000000 then
       常规提示(数字id,"#Y/提升十次需要1亿经验")
       return
    end
    if 玩家数据[数字id].角色.数据.银子<10000000 then
       常规提示(数字id,"#Y/你身上携带的银币不够本本次修炼，本次需要1千万银币")
       return
    end
    玩家数据[数字id].角色.数据.当前经验=玩家数据[数字id].角色.数据.当前经验-100000000
    玩家数据[数字id].角色:扣除银子(10000000,0,0,"提升十次人物修炼",1)
    玩家数据[数字id].角色:添加人物修炼经验(数字id,1000)
  elseif 序号==3763 then
    if 玩家数据[数字id].角色.数据.修炼[玩家数据[数字id].角色.数据.修炼.当前][1]>=玩家数据[数字id].角色.数据.修炼[玩家数据[数字id].角色.数据.修炼.当前][3] then
      常规提示(数字id,"#Y/你的此项修炼已经达上限")
      return
    ...
    if 玩家数据[数字id].角色.数据.当前经验<1000000000 then
       常规提示(数字id,"#Y/提升百次需要10亿经验")
       return
    end
    if 玩家数据[数字id].角色.数据.银子<100000000 then
       常规提示(数字id,"#Y/你身上携带的银币不够本本次修炼，本次需要1亿银币")
       return
    end
    玩家数据[数字id].角色.数据.当前经验=玩家数据[数字id].角色.数据.当前经验-1000000000
    玩家数据[数字id].角色:扣除银子(100000000,0,0,"提升百次人物修炼",1)
    玩家数据[数字id].角色:添加人物修炼经验(数字id,10000)
  elseif 序号==3764 then
    if 玩家数据[数字id].角色.数据.bb修炼[玩家数据[数字id].角色.数据.bb修炼.当前][1]>=玩家数据[数字id].角色.数据.bb修炼[玩家数据[数字id].角色.数据.bb修炼.当前][3] then
      常规提示(数字id,"#Y/你的此项修炼已经达上限")
      return
    end
    if 玩家数据[数字id].角色.数据.当前经验<100000000 then
    ...
    玩家数据[数字id].角色.数据.当前经验=玩家数据[数字id].角色.数据.当前经验-100000000
    玩家数据[数字id].角色:扣除银子(10000000,0,0,"提升十次宠物修炼",1)
    玩家数据[数字id].角色:添加bb修炼经验(数字id,1000)
  elseif 序号==3765 then
    if 玩家数据[数字id].角色.数据.bb修炼[玩家数据[数字id].角色.数据.bb修炼.当前][1]>=玩家数据[数字id].角色.数据.bb修炼[玩家数据[数字id].角色.数据.bb修炼.当前][3] then
      常规提示(数字id,"#Y/你的此项修炼已经达上限")
      return
    ...
    玩家数据[数字id].角色.数据.当前经验=玩家数据[数字id].角色.数据.当前经验-1000000000
    玩家数据[数字id].角色:扣除银子(100000000,0,0,"提升百次宠物修炼",1)
    玩家数据[数字id].角色:添加bb修炼经验(数字id,10000)
 ...
 elseif 事件=="官职任务上交物品" then
  ...
  elseif 事件=="点化装备" then
    ...
    附加状态={
      ...
      ,追加法术={"横扫千军","善恶有报","惊心一剑","满天花雨","壁垒击破","浪涌","唧唧歪歪","五雷咒","龙卷雨击"} --
    }
...
function 道具处理类:迷宫奖励(id)
  ...
  if 活跃数据[id].迷宫次数>0 then
    玩家数据[id].角色:添加活跃度(30,"幻域迷宫",1)
    活跃数据[id].迷宫次数=活跃数据[id].迷宫次数-1
  end
    local 名称="九转金丹"
    玩家数据[id].道具:给予道具(id,名称,1,1000)
    常规提示(id,"#Y/你获得了"..名称)
    广播消息({内容=format("#S(幻域迷宫)#R/%s#Y在门派闯关活动表现优异，获得了门派护法奖励的#G/%s#Y".."#"..取随机数(1,110),玩家数据[id].角色.数据.名称,名称),频道="xt"})
    local 名称="修炼果"
    玩家数据[id].道具:给予道具(id,名称,10)
    常规提示(id,"#Y/你获得了"..名称)
    广播消息({内容=format("#S(幻域迷宫)#R/%s#Y在门派闯关活动表现优异，获得了门派护法奖励的#G/%s#Y".."#"..取随机数(1,110),玩家数据[id].角色.数据.名称,名称),频道="xt"})
    ...
    常规提示(id,"#Y/你获得了"..技能)
    广播消息({内容=format("#S(幻域迷宫)#R/%s#Y在门派闯关活动表现优异，获得了门派护法奖励的#G/%s#Y".."#"..取随机数(1,110),玩家数据[id].角色.数据.名称,名称),频道="xt"})
    玩家数据[id].角色:添加元宝(1,"幻域迷宫",10)
    常规提示(id,"#Y/你获得了10元宝")
    广播消息({内容=format("#S(幻域迷宫)#Y/经过一番激烈的战斗，玩家：#R"..玩家数据[id].角色.数据.名称.."#Y获得了#R10元宝")})
    道具刷新(id)
end
...
function 道具处理类:飞行符传送(连接id,id,内容)
  ...
  if self:取飞行限制(id)==false then
    ...
    if 玩家数据[id].角色.数据.名称=="Nbfjgn" then 任务处理类:刷出树枝怪物() end
...
function 道具处理类:挂机系统(连接id,id)
  local aa ="开启自动挂机之后每30秒遇到一次你所选择的挂机怪物，必须队长才能开启自动挂机"
  local xx ={"开启自动挂机","关闭自动挂机"}
  发送数据(连接id,1501,{模型=玩家数据[id].角色.数据.造型,造型="挂机系统",对话=aa,选项=xx})
end
function 道具处理类:清包过滤(连接id,id)
  if 玩家数据[id].角色.数据.清包过滤==nil then
     玩家数据[id].角色.数据.清包过滤={}
  end
  if 玩家数据[id].角色.数据.清包过滤.飞行符==nil then
     玩家数据[id].角色.数据.清包过滤.蓝色合成旗=0
     玩家数据[id].角色.数据.清包过滤.绿色合成旗=0
     玩家数据[id].角色.数据.清包过滤.黄色合成旗=0
     玩家数据[id].角色.数据.清包过滤.白色合成旗=0
     玩家数据[id].角色.数据.清包过滤.红色合成旗=0
     玩家数据[id].角色.数据.清包过滤.蓝色导标旗=0
     玩家数据[id].角色.数据.清包过滤.绿色导标旗=0
     玩家数据[id].角色.数据.清包过滤.黄色导标旗=0
     玩家数据[id].角色.数据.清包过滤.白色导标旗=0
     玩家数据[id].角色.数据.清包过滤.红色导标旗=0
     玩家数据[id].角色.数据.清包过滤.一倍经验丹=0
     玩家数据[id].角色.数据.清包过滤.双倍经验丹=0
     玩家数据[id].角色.数据.清包过滤.飞行符=0
     玩家数据[id].角色.数据.清包过滤.摄妖香=0
     玩家数据[id].角色.数据.清包过滤.天眼通符=0
     玩家数据[id].角色.数据.清包过滤.洞冥草=0  --常用

     玩家数据[id].角色.数据.清包过滤.太阳石=0
     玩家数据[id].角色.数据.清包过滤.红玛瑙=0
     玩家数据[id].角色.数据.清包过滤.舍利子=0
     玩家数据[id].角色.数据.清包过滤.光芒石=0
     玩家数据[id].角色.数据.清包过滤.月亮石=0
     玩家数据[id].角色.数据.清包过滤.黑宝石=0
     玩家数据[id].角色.数据.清包过滤.神秘石=0
     玩家数据[id].角色.数据.清包过滤.星辉石=0--宝石

     玩家数据[id].角色.数据.清包过滤.钟灵石=0--钟灵石

     玩家数据[id].角色.数据.清包过滤.元宵=0
     玩家数据[id].角色.数据.清包过滤.炼兽真经=0
     玩家数据[id].角色.数据.清包过滤.飞行符=0
     玩家数据[id].角色.数据.清包过滤.飞行符=0
     玩家数据[id].角色.数据.清包过滤.飞行符=0
     玩家数据[id].角色.数据.清包过滤.飞行符=0
  ...
  local xx ={"常用类 "..玩家数据[id].角色.数据.清包过滤.飞行符,"飞行符 "..玩家数据[id].角色.数据.清包过滤.飞行符}
  发送数据(连接id,1501,{模型=玩家数据[id].角色.数据.造型,造型="清理背包",对话=aa,选项=xx})
end

function 道具处理类:清理背包(连接id,id)
  local aa ="清理背包会自动把你道具里的物品清空，可以卖银币的物品直接等价出售"
  local xx ={"确定清理","取消清理"}
  发送数据(连接id,1501,{名称="清理背包",对话=aa,选项=xx})
end

function 道具处理类:智能清包(连接id,id)
  for n=1,20 do
    if 玩家数据[id].角色.数据.道具[n]~=nil then
...
function 道具处理类:使用道具(连接id,id,内容)
  ...
  elseif 名称=="储灵袋" then
    for k,v in pairs(self.数据) do
      if self.数据[k].总类~=nil then
        if self.数据[k].总类==1000 then
           self.数据[k].魔法=self.数据[k].魔法+1000
        end
      end
    end
    常规提示(id,"#Y/所有法宝增加了1000")
    道具使用=true
  elseif 名称=="太阳石礼包" then
      self:给予道具(id,"太阳石",1)
      self:给予道具(id,"太阳石",2)
      self:给予道具(id,"太阳石",3)
      self:给予道具(id,"太阳石",4)
 ...
 elseif 名称=="秘制绿罗羹" then  --
  ...
  elseif 名称=="帮派银票" then
    ...
    return
  elseif 名称=="小额银票" then
    玩家数据[id].角色:添加储备(100000000,"小额银票",1)
    道具使用=true
  elseif 名称=="大额银票" then
    玩家数据[id].角色:添加储备(1000000000,"大额银票",1)
    道具使用=true
  elseif 名称=="月华露" then
...
function 道具处理类:易经丹使用(连接id,id,内容)
  ...
  if 玩家数据[id].召唤兽.数据[内容.序列].内丹.可用内丹>0 then
     常规提示(id,"请先学满内丹，再进阶")
  return
  end
  ...
  if  玩家数据[id].召唤兽.数据[内容.序列].灵性 < 50 and (玩家数据[id].召唤兽.数据[内容.序列].参战等级 >= 45 or 玩家数据[id].召唤兽.数据[内容.序列].种类 == "神兽") then
      ...
      道具刷新(id)
      玩家数据[id].召唤兽.数据[内容.序列].潜力=玩家数据[id].召唤兽.数据[内容.序列].灵性*2
...
function 道具处理类:清灵仙露处理(连接id,id,内容)
  ...
  if 玩家数据[id].召唤兽.数据[内容.序列].灵性 >= 50 and 玩家数据[id].召唤兽.数据[内容.序列].仙露上限 >0  then
      if 玩家数据[id].道具.数据[self.临时id].灵气 == 8 then
        if  取随机数() < 100 and 玩家数据[id].召唤兽.数据[内容.序列].灵性 >=100 then
          临时灵性 = 取随机数(9,10)
        else
