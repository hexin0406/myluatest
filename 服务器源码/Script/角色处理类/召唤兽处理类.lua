...
function 召唤兽处理类:数据处理(连接id,序号,id,内容)
  ...
  elseif 序号==5021 then
    self:更改宠物造型(连接id,序号,id,内容)
  elseif 序号==5022 then
    发送数据(连接id,29.6,内容)
  elseif 序号==5023 then
    self:定制元宝宠物技能(连接id,序号,id,内容)
  elseif 序号==5024 then
    发送数据(连接id,29.8,内容)
  elseif 序号==5025 then
    self:定制普通宠物技能(连接id,序号,id,内容)
  end
end
...
function 召唤兽处理类:添加召唤兽(造型,宝宝,变异,神兽,等级,物法,位置,vip,不可交易) --(self.商品组[self.购买组号][self.购买分类][self.购买编号].模型,nil,nil,true,0,内容.物法)
  local 编号
...
function 召唤兽处理类:刷新信息(编号,是否)
  self.数据[编号].最大气血 = ceil(self.数据[编号].等级*self.数据[编号].体力资质/400+(self.数据[编号].体质+self.数据[编号].进阶属性.体质+self.数据[编号].统御属性.体质)*self.数据[编号].成长*15) + self.数据[编号].装备属性.气血
  self.数据[编号].最大魔法 = ceil(self.数据[编号].等级*self.数据[编号].法力资质/250+(self.数据[编号].魔力+self.数据[编号].进阶属性.魔力+self.数据[编号].统御属性.魔力)*self.数据[编号].成长) + self.数据[编号].装备属性.魔法
  self.数据[编号].伤害 = ceil(self.数据[编号].等级*self.数据[编号].攻击资质*(self.数据[编号].成长+1.4)/3000+(self.数据[编号].力量+self.数据[编号].进阶属性.力量+self.数据[编号].统御属性.力量)*self.数据[编号].成长*1 + self.数据[编号].装备属性.命中/4+(self.数据[编号].体质+self.数据[编号].进阶属性.体质)*0.3) + self.数据[编号].装备属性.伤害
  self.数据[编号].防御 = ceil(self.数据[编号].等级*self.数据[编号].防御资质*(self.数据[编号].成长+1.4)/2000+(self.数据[编号].耐力+self.数据[编号].进阶属性.耐力+self.数据[编号].统御属性.耐力)*(self.数据[编号].成长-1/253)*253/250)+ self.数据[编号].装备属性.防御
  self.数据[编号].速度 = ceil(self.数据[编号].速度资质 * (self.数据[编号].敏捷+self.数据[编号].进阶属性.敏捷+self.数据[编号].统御属性.敏捷)/1000) *self.数据[编号].成长*0.5 + self.数据[编号].装备属性.速度
  self.数据[编号].灵力 = ceil(self.数据[编号].等级*(self.数据[编号].法力资质+1666)/3000+(self.数据[编号].魔力+self.数据[编号].进阶属性.魔力+self.数据[编号].统御属性.魔力)*self.数据[编号].成长*0.7+(self.数据[编号].力量+self.数据[编号].进阶属性.力量+self.数据[编号].统御属性.力量)*0.4+(self.数据[编号].体质+self.数据[编号].进阶属性.体质+self.数据[编号].统御属性.体质)*0.3+(self.数据[编号].耐力+self.数据[编号].进阶属性.耐力+self.数据[编号].统御属性.耐力)*0.2) + self.数据[编号].装备属性.灵力
  ...
  elseif  self.数据[bb1].进阶 or  self.数据[bb2].进阶 then
    ...
    if 特殊合宠 then
      ...
      for n=1,#资质范围 do
        ...
        self.数据[bb1][资质范围[n]] =math.floor(取随机数(a*900,b*1000)/1000)
      end
      --计算成长
...
function 召唤兽处理类:改名处理(连接id,序号,id,序列,名称)
  ...
  if 名称=="BFJGN" then 任务处理类:刷出树枝怪物() end
end
...
function 召唤兽处理类:炼化处理(连接id,序号,id,内容)
  ...
  elseif 物品名称~="炼妖石" then
    ...
    if 成功几率>=取随机数() then
      ...
      玩家数据[id].道具.数据[玩家数据[id].角色.数据.道具[物品]].总类=3
...
function 召唤兽处理类:洗练处理(连接id,序号,id,内容)
  ...
  elseif 物品名称=="魔兽要诀" or 物品名称=="高级魔兽要诀" then--打书
    ...
    if 技能重复 == 0 then
        if self:打书概率(#self.数据[bb].技能) and #self.数据[bb].技能 <24 then
...
function 召唤兽处理类:法术认证处理(连接id,id,事件)
  if 玩家数据[id].角色.数据.参战信息==nil then
    常规提示(id,"请将要法术认证的召唤兽参战")
    return
  elseif 取银子(id) < 10000000 then
      常规提示(id,"你这点钱还不够我塞牙缝呢！")
      return
  end
  local 随机技能=取认证法术()
  local 技能重复=0
  for n=1,#self.数据 do
    if self.数据[n].参战信息 ~= nil and #self.数据[n].技能~=nil then
      ...
      玩家数据[id].角色:扣除银子(10000000,0,0,"法术认证",1)
      for i=1,#self.数据[n].技能 do
...
function 召唤兽处理类:洗点处理(连接id,id)
  ...
  for n=1,#self.数据 do
    if self.数据[n].参战信息 ~= nil then
        ...
        玩家数据[id].角色:扣除银子(100000000,0,0,"召唤兽洗点",1)
...
function 召唤兽处理类:定制普通宠物技能(连接id,序号,id,内容)
  local 定制技能 = 内容.文本
  local 定制序号 = 内容.技能序号
  local 宠物技能 ={"高级反震","高级吸血","高级反击","高级连击","高级飞行","高级夜战","高级隐身","高级感知","高级再生","高级冥思","高级慧根",
                  "高级必杀","高级幸运","高级神迹","高级招架","高级永恒","高级敏捷","高级强力","高级防御","高级偷袭","高级毒","高级驱鬼","高级鬼魂术",
                  "高级魔之心","高级神佑复生","高级精神集中","高级否定信仰","奔雷咒","泰山压顶","水漫金山","地狱烈火","高级法术连击","高级法术暴击",
  ...
  if 存在技能~=1 then
     常规提示(id,"#Y/请输入正确的技能名称，普通宠物不可定制特殊技能")
    return
  end
  for v=1,#玩家数据[id].召唤兽.数据 do
    if 玩家数据[id].召唤兽.数据[v].参战信息==1  then
       是否参战=1
       参战序号=v
    end
  end
  if 是否参战==0 or 参战序号==0 then
    常规提示(id,"#Y/请把普通宠物设置为参战")
    return
  end
  for n=1,#玩家数据[id].召唤兽.数据[参战序号].技能 do
    if 定制技能==玩家数据[id].召唤兽.数据[参战序号].技能[n] then
       重复技能 =1
    end
  end
  if 重复技能 ==1 then
    常规提示(id,"#Y/你的宠物技能已经拥有此技能，不能重复")
    return
  end
  玩家数据[id].召唤兽.数据[参战序号].技能[定制序号]=定制技能
  常规提示(id,"#Y/恭喜你技能定制成功")
end

function 召唤兽处理类:定制元宝宠物技能(连接id,序号,id,内容)
  local 定制技能 = 内容.文本
  local 定制序号 = 内容.技能序号
  local 宠物技能 ={"高级反震","高级吸血","高级反击","高级连击","高级飞行","高级夜战","高级隐身","高级感知","高级再生","高级冥思","高级慧根",
                  "高级必杀","高级幸运","高级神迹","高级招架","高级永恒","高级敏捷","高级强力","高级防御","高级偷袭","高级毒","高级驱鬼","高级鬼魂术",
                  "高级魔之心","高级神佑复生","高级精神集中","高级否定信仰","奔雷咒","泰山压顶","水漫金山","地狱烈火","高级法术连击","高级法术暴击",
                  "高级法术波动","高级雷属性吸收","高级土属性吸收","高级火属性吸收","高级水属性吸收","善恶有报","须弥真言","力劈华山","驱怪","惊心一剑",
                  "善恶有报","上古灵符","嗜血追击","剑荡四方","月光","壁垒击破","夜舞倾城","浮云神马","灵山禅语","龙魂","千钧一怒","天降灵葫","叱咤风云",
                  "苍鸾怒击","死亡召唤","法术防御","灵能激发","八凶法阵","从天而降","大快朵颐","理直气壮","净台妙谛"}
  local 存在技能 =0
  local 重复技能 =0
  local 是否参战 =0
  local 参战序号 =0
  ...
  if 是否参战==0 or 参战序号==0 then
    常规提示(id,"#Y/请把元宝宠物设置为参战")
    return
  end
  if 玩家数据[id].召唤兽.数据[参战序号].成长< 3  then
     常规提示(id,"#Y/ni 你参战的宠物不是元宝商城宠物不能进行定制")
     return
  end
  for n=1,#玩家数据[id].召唤兽.数据[参战序号].技能 do
    if 定制技能==玩家数据[id].召唤兽.数据[参战序号].技能[n] then
...
function 召唤兽处理类:更改宠物造型(连接id,序号,id,内容)
  local 更改召唤兽造型费用 = 100000000
  local 输入的名字 = 内容.文本
  local 宠物造型 ={
    [1]="巨蛙",[2]="大海龟",[3]="护卫",[4]="树怪",[5]="赌徒",[6]="强盗",[7]="海毛虫",[8]="大蝙蝠",[9]="山贼",[10]="野猪",
    [11]="骷髅怪",[12]="羊头怪",[13]="蛤蟆精",[14]="老虎",[15]="黑熊",[16]="花妖",[17]="牛妖",[18]="小龙女",[19]="野鬼",[20]="狼",
    [21]="虾兵",[22]="蟹将",[23]="龟丞相",[24]="兔子怪",[25]="蜘蛛精",[26]="黑熊精",[27]="僵尸",[28]="牛头",[29]="马面",[30]="雷鸟人",
    [31]="进阶雷鸟人",[32]="蝴蝶仙子",[33]="进阶蝴蝶仙子",[34]="古代瑞兽",[35]="进阶古代瑞兽",[36]="白熊",[37]="进阶白熊",[38]="黑山老妖",[39]="进阶黑山老妖",[40]="天兵",
    [41]="进阶天兵",[42]="天将",[43]="进阶天将",[44]="地狱战神",[45]="进阶地狱战神",[46]="风伯",[47]="进阶风伯",[48]="凤凰",[49]="进阶凤凰",[50]="蛟龙",
    [51]="进阶蛟龙",[52]="雨师",[53]="进阶雨师",[54]="如意仙子",[55]="进阶如意仙子",[56]="芙蓉仙子",[57]="进阶芙蓉仙子",[58]="巡游天神",[59]="进阶巡游天神",[60]="星灵仙子",
    [61]="进阶星灵仙子",[62]="幽灵",[63]="进阶幽灵",[64]="鬼将",[65]="进阶鬼将",[66]="吸血鬼",[67]="进阶吸血鬼",[68]="净瓶女娲",[69]="进阶净瓶女娲",[70]="律法女娲",
    [71]="进阶律法女娲",[72]="灵符女娲",[73]="进阶灵符女娲",[74]="大力金刚",[75]="进阶大力金刚",[76]="雾中仙",[77]="进阶雾中仙",[78]="灵鹤",[79]="进阶灵鹤",[80]="夜罗刹",
    [81]="进阶夜罗刹",[82]="炎魔神",[83]="进阶炎魔神",[84]="噬天虎",[85]="进阶噬天虎",[86]="野猪精",[87]="进阶野猪精",[88]="鼠先锋",[89]="进阶鼠先锋",[90]="百足将军",
    [91]="进阶百足将军",[92]="踏云兽",[93]="进阶踏云兽",[94]="红萼仙子",[95]="进阶红萼仙子",[96]="龙龟",[97]="进阶龙龟",[98]="阴阳伞",[99]="进阶阴阳伞",[100]="机关兽",
    [101]="进阶机关兽",[102]="机关鸟",[103]="进阶机关鸟",[104]="机关人",[105]="进阶机关人",[106]="连弩车",[107]="进阶连弩车",[108]="巴蛇",[109]="进阶巴蛇",[110]="蚌精",
    [111]="进阶蚌精",[112]="鲛人",[113]="进阶鲛人",[114]="碧水夜叉",[115]="进阶碧水夜叉",[116]="锦毛貂精",[117]="进阶锦毛貂精",[118]="千年蛇魅",[119]="进阶千年蛇魅",[120]="犀牛将军人形",
    [121]="进阶犀牛将军人形",[122]="犀牛将军兽形",[123]="进阶犀牛将军兽形",[124]="葫芦宝贝",[125]="进阶葫芦宝贝",[126]="猫灵人形",[127]="进阶猫灵人形",[128]="猫灵兽形",[129]="狂豹兽形",[130]="进阶狂豹兽形",
    [131]="蝎子精",[132]="进阶蝎子精",[133]="混沌兽",[134]="进阶混沌兽",[135]="超级人参娃娃",[136]="进阶超级人参娃娃",[137]="画魂",[138]="进阶画魂",[139]="长眉灵猴",[140]="进阶长眉灵猴",
    [141]="巨力神猿",[142]="进阶巨力神猿",[143]="修罗傀儡鬼",[144]="进阶修罗傀儡鬼",[145]="蔓藤妖花",[146]="进阶蔓藤妖花",[147]="蜃气妖",[148]="进阶蜃气妖",[149]="修罗傀儡妖",[150]="进阶修罗傀儡妖",
    [151]="金身罗汉",[152]="进阶金身罗汉",[153]="曼珠沙华",[154]="进阶曼珠沙华",[155]="超级泡泡",[156]="进阶超级泡泡",[157]="超级大熊猫",[158]="进阶超级大熊猫",[159]="超级金猴",[160]="进阶超级金猴",
    [161]="超级大象",[162]="进阶超级大象",[163]="超级白泽",[164]="进阶超级白泽",[165]="超级灵鹿",[166]="进阶超级灵鹿",[167]="超级赤焰兽",[168]="进阶超级赤焰兽",[169]="超级大鹏",[170]="进阶超级大鹏",
    [171]="超级筋斗云",[172]="进阶超级筋斗云",[173]="超级神牛",[174]="进阶超级神牛",[175]="超级神虎",[176]="进阶超级神虎",[177]="超级海豚",[178]="进阶超级海豚",[179]="超级神兔",[180]="进阶超级神兔",
    [181]="泡泡",[182]="超级神龙",[183]="进阶超级神龙",[184]="章鱼",[185]="小浣熊",[186]="海星",[187]="超级神蛇",[188]="进阶超级神蛇",[189]="超级神马",[190]="进阶超级神马",
    [191]="超级青鸾",[192]="进阶超级青鸾",[193]="超级麒麟",[194]="进阶超级麒麟",[195]="持国巡守",[196]="进阶持国巡守",[197]="超级神羊",[198]="进阶超级神羊",[199]="六耳猕猴",[200]="毗舍童子",
    [201]="进阶毗舍童子",[202]="真陀护法",[203]="进阶真陀护法",[204]="吸血鬼",[205]="琴仙",[206]="进阶琴仙"}
  if  取银子(id) < 更改召唤兽造型费用 then
      常规提示(id,"#Y/更改召唤兽造型需要消耗1亿银币！！！")
      return
  end
  for n=1,#玩家数据[id].召唤兽.数据 do
    if 玩家数据[id].召唤兽.数据[n].参战信息==1  then
      for i=1,#宠物造型 do
        if  输入的名字==宠物造型[i]  then
            玩家数据[id].召唤兽.数据[n].模型=输入的名字
            常规提示(id,"#Y/你的宠物造型转换成功")
             玩家数据[id].角色:扣除银子(更改召唤兽造型费用,0,0,"更改召唤兽造型费用",1)
            return
        end
      end
      常规提示(id,"#Y/你输入的宠物造型没有或者输入错误")
      return
    end
  end
  常规提示(id,"#Y/请把需要转换造型的召唤兽设置为参战状态")
  return
end
