...
function 地图处理类:更改炫彩(id,xc,xcz)
  ...
  for n, v in pairs(self.地图玩家[地图编号]) do
    if n~=id and self:取同一地图(地图编号,id,n,1)  then
      发送数据(玩家数据[n].连接id,1032,{id=id,炫彩=xc,炫彩组=xcz})
    end
  end
end
...
function 地图处理类:更新锦衣(id,锦衣)
  ...
  for n, v in pairs(self.地图玩家[地图编号]) do
    if n~=id and self:取同一地图(地图编号,id,n,1) then
      发送数据(玩家数据[n].连接id,1014,{id=id,锦衣=锦衣,锦衣隐藏=玩家数据[id].角色.数据.锦衣隐藏+0})
    end
  end
end
...
function 地图处理类:更新坐骑(id,坐骑)
  ...
  for n, v in pairs(self.地图玩家[地图编号]) do
    if n~=id and self:取同一地图(地图编号,id,n,1)  then
      发送数据(玩家数据[n].连接id,1019,{id=id,坐骑=坐骑,坐骑隐藏=玩家数据[id].角色.数据.坐骑隐藏+0})
    end
  end
end
...
function 地图处理类:更新长安城城主(任务id,id组)
  for n, v in pairs(任务数据) do
    if 任务数据[n].类型 == 80001 then
      self:删除单位(任务数据[n].地图编号,任务数据[n].单位编号)
      任务数据[n] = nil
    end
  end
  for i=1,#id组 do
   自定义怪[i]={}
   自定义怪[i].名称 = 玩家数据[id组[i]].角色.数据.名称
   自定义怪[i].模型 = 玩家数据[id组[i]].角色.数据.模型
   if 玩家数据[id组[i]].角色.数据.穿戴锦衣 ~= nil then
     自定义怪[i].锦衣 = 玩家数据[id组[i]].角色.数据.穿戴锦衣
   end
   if 玩家数据[id组[i]].角色.数据.装备[3] ~= nil then
     自定义怪[i].武器 = 玩家数据[id组[i]].道具.数据[玩家数据[id组[i]].角色.数据.装备[3]].名称
     自定义怪[i].武器等级 = 玩家数据[id组[i]].道具.数据[玩家数据[id组[i]].角色.数据.装备[3]].级别限制
     自定义怪[i].染色组 = 玩家数据[id组[i]].道具.数据[玩家数据[id组[i]].角色.数据.装备[3]].染色组
     自定义怪[i].染色方案 = 玩家数据[id组[i]].道具.数据[玩家数据[id组[i]].角色.数据.装备[3]].染色方案
    end
  end
  写出文件([[tysj/自定义怪.txt]],table.tostring(自定义怪))
  任务处理类:全服功臣()
end

function 地图处理类:更新自定义怪(任务id,id组)
  for n, v in pairs(任务数据) do
    if 任务数据[n].类型 == 80000 then
      self:删除单位(任务数据[n].地图编号,任务数据[n].单位编号)
      任务数据[n] = nil
    end
  end
