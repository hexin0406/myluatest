...
-- @Last Modified time: 2020-08-24 15:41:01
...
function 网络处理类:数据处理(id,源)
  ...
  if self.数据.硬盘 ~= nil then
    if f函数.读配置(程序目录 .. "机器码封禁.ini", "机器码", self.数据.硬盘)=="1" then
      __S服务:输出(string.format("封禁机器码的客户进入试图进入(%s)", id))
      发送数据(id,997,"")
      return 0
    end
  end
  ...
  elseif 序号>6200 and 序号 <=6300 then
    管理工具类:数据处理(self.数据.数字id+0,序号,self.数据)
  --elseif 序号>7000 and 序号 <=7500 then
  --  场景类_NPC对话处理:场景类_NPC对话处理(self.数据.数字id+0,序号,self.数据)
