...
function 系统处理类:数据处理(id,序号,内容)
  ...
  elseif 序号 == 92.1 then
    ...
    local 次数=内容.次数
    local 银子
    local 一次银子 = 玩家数据[id].角色.数据.坐骑列表[序列].修炼.速度修炼[1]*10000000
    local 十次银子 = (玩家数据[id].角色.数据.坐骑列表[序列].修炼.速度修炼[1]*10+45)*10000000
    local 五十次银子 = (玩家数据[id].角色.数据.坐骑列表[序列].修炼.速度修炼[1]*50+1225)*10000000
    if 玩家数据[id].角色.数据.坐骑列表[序列]==nil then
      常规提示(id,"#Y你没有这样的坐骑")
      return
    else
  ...
  elseif 序号 == 102 then
    if 玩家数据[内容.数字id].角色:扣除元宝(50,"人物炫彩",内容.数字id)  then
      ...
      常规提示(内容.数字id,"角色染色成功")
      发送数据(玩家数据[内容.数字id].连接id,122,{炫彩=玩家数据[内容.数字id].角色.数据.炫彩,炫彩组=玩家数据[内容.数字id].角色.数据.炫彩组})
...
function 系统处理类:创建账号(id,序号,内容)
  ...
  elseif f函数.文件是否存在([[data/]]..内容.账号)==false then  --未创建存档
    ...
    f函数.写配置(程序目录..[[data\]]..内容.账号..[[\账号信息.txt]],"账号配置","仙玉","99999999")
