...
-- @Last Modified time: 2020-06-04 20:19:33
--======================================================================--
local 场景类_钓鱼 = class()
...
function 场景类_钓鱼:取鱼资源(id)
	local y = {
		-- 左至右 右至左
		{{0xC58184F5,0x2BA2E98C},{0x692B53CB,0x428E63B2},"草鱼",1.2},
		{{0x00FFE220,0x03DCBE41},{0xDF44710B,0x3E138B56},"甲鱼",0.8}
	}
	return y[id]
end
...
function 场景类_钓鱼:打开(渔场,人员)
	if self.可视 then
		...
		self.资源组 = {
			...
			[2] = 自适应.创建(1,1,603,18,1,3,nil,18),
			[3] = 按钮.创建(自适应.创建(18,4,16,16,4,3),0,0,4,true,true),
			-- 背景素材
			[5] = 资源:载入('wzife.wd4',"网易WDF动画",0x24E41DF4),
			[6] = {},
			[8] = {},
			...
			[13] = 资源:载入('wzife.wd4',"网易WDF动画",0x82FA515B),
			[14] = 资源:载入('wzife.wd4',"网易WDF动画",0x484A618B),
			[15] = 资源:载入('wzife.wd4',"网易WDF动画",0xA0C042CD),
		...
		if 渔场 == 1 then
			self.资源组[4] = tp.资源:载入('wzife.wd4',"网易WDF动画",0x25029DA4)
			self.资源组[7] = tp.资源:载入('wzife.wd4',"网易WDF动画",0x3E36C7F5)
			self.渔场 = "傲来渔场"
		else
			self.资源组[4] = tp.资源:载入('wzife.wd4',"网易WDF动画",0x0C4965C0)
			self.资源组[7] = tp.资源:载入('wzife.wd4',"网易WDF动画",0x7CF21DD2)
			self.渔场 = "长寿渔场"
		end
		self.人员 = 人员
		for i=1,self.人员 do
			...
			for n=1,5 do
				self.资源组[6][i][3][n] = {}
				self.资源组[6][i][3][n][1] = tp.资源:载入('wzife.wd4',"网易WDF动画",yg[n])
			end
			self.资源组[6][i][4] = 1
		end
		self.可视1 = true
...
function 场景类_钓鱼:显示(dt,x,y)
	...
	tp.窗口标题背景_:置区域(0,0,85,16)
	tp.窗口标题背景_:显示(self.x+265,self.y+3)
	zts1:置字间距(5)
	zts1:显示(self.x+271,self.y+3,self.渔场)
	zts1:置字间距(0)
	self.资源组[3]:显示(self.x+612,self.y+5)
	for i=1,self.人员 do
	...
	for i=1,#self.资源组[8] do
		if self.资源组[8][i] ~= nil then
			...
			if self.资源组[8][i] ~= nil and self.资源组[6][1][4] == 3 and self.资源组[6][1][5][1] == nil and self.资源组[8][i][8] == nil then
				...
				if self.资源组[8][i][4].x >= zxz and self.资源组[8][i][4].x <= zdz then
	...
	if self.下次加入鱼时间 <= 0 then
		...
		for i=1,self.下次加入鱼数量 do
			...
			zy[1] = tp.资源:载入('wzife.wd4',"网易WDF动画",z[1])
			zy[2] = tp.资源:载入('wzife.wd4',"网易WDF动画",z[2])
			zy[3] = u[3]
			if f == 1 then
				x =  zy[1].宽度+10
			else
