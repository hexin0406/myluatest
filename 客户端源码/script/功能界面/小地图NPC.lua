...
function 场景类_小地图NPC:打开()
	if self.可视 then
		...
		self.NPC假人={}
		self.资源组[7]:置起始点(0)
		for n=1,#tp.场景.假人[tp.当前地图] do
			if tp.场景.假人[tp.当前地图][n].小地图名称颜色 ~=nil and tp.场景.假人[tp.当前地图][n].小地图名称颜色 >=0 then
		    	insert(self.NPC假人,tp.场景.假人[tp.当前地图][n])
			end
		end
	end
end
...
function 场景类_小地图NPC:显示(dt,x,y)
	...
	fonts1:置字间距(2)
	fonts1:显示(self.x+176,self.y+3,取地图名称(tp.当前地图))
	self.状态=1
	if self.状态 == 1 then
