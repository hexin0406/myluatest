local GUI基类  		= require("ggeui/基类")
local GUI按钮类  	= require("ggeui/按钮类")
local GUI列表类 	= require("ggeui/列表类")
local GUI多列表类 	= require("ggeui/多列表类")
local GUI滑块类 	= require("ggeui/滑块类")
local GUI进度类		= require("ggeui/进度类")
local GUI丰富文本 	= require("ggeui/丰富文本")
local GUI树形类 	= require("ggeui/树形类")
local GUI自适应类 	= require("ggeui/自适应类")
local GUI网格类 	= require("ggeui/网格类")
local GUI输入类 	= require("ggeui/输入类")
local GUI输入类1 	= require("ggeui/输入类1")
local GUI控件类
...
function GUI创建基类:创建输入1(名称,...)
	if self._子控件[名称] then error(名称..":此输入已存在，不能重复创建.")end
	local 对象 				= GUI输入类1.创建(名称,...)
	self._子控件[名称] 		= 对象
	对象._父控件 			= self
	对象._层次 				= #self._子控件
...
function GUI控件类:置可视(v,sub)
	self:运行父函数(GUI基类,'置可视',v,sub)
	if self:是否可视() then
		if self._类型 == '窗口' then
			self:_消息事件('窗口移动',self._x,self._y)
		end
...
function GUI控件类:_消息事件(...)
	local 类型,x,y = ...
	if (self:是否加载() and self:是否可视()) or 类型 == '窗口移动' then
		--父先收消息,以便拦截
		if (self:是否可视() or 类型 == '可视事件') and self.消息事件 and self:消息事件(...) then
			return true
		end
		...
		for i=#self._子控件,1,-1 do
			...
			elseif (v:是否可视() or 类型 == '可视事件') and v:_消息事件(...) then
				return true
			end
		end
	    ...
	    if self:发送消息('消息结束') then--拦截的呢消息
	    	return true
		elseif not self:是否禁止() and (类型 == '鼠标移动' or 类型 == '左键按下' or 类型 == '左键弹起'or 类型 == '右键按下' or 类型 == '右键弹起') then
