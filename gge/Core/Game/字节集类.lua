...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2017-03-24 17:25:00
...
function _字节集:取文本(i,len)
	return ffi.string(self.bin+(i or 0),len)
end
function _字节集:取整数(i)
	if i and i+3 <= self.len then
		return bit.bor(bit.lshift(self.bin[i+3], 24),bit.lshift(self.bin[i+2], 16),bit.lshift(self.bin[i+1], 8),self.bin[i])
	end
	return 0
end
function _字节集:取短整数(i)
	if i and i+1 <= self.len then
	    return bit.bor(bit.lshift(self.bin[i+1], 8),self.bin[i])
	end
	return 0
end
-- function _字节集:取小数(i)
...
function _字节集:写整数(i,v)
	if i and i+3 < self.len then
		self.bin[i] 	= bit.band(v,0xFF)
		self.bin[i+1] 	= bit.band(bit.rshift(v, 8),0xFF)
		self.bin[i+2] 	= bit.band(bit.rshift(v, 16),0xFF)
		self.bin[i+3] 	= bit.band(bit.rshift(v, 24),0xFF)
	end
end
...
function _字节集:填充(v,off,len)
	ffi.fill(self.bin+(off or 0),len or self.len ,v or 0)
end
