...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2017-01-06 19:47:40
...
function point:取弧度(p)
	...
	if(p.y ==y and p.x==x)then
		return 0
	elseif(p.y >=y and p.x<=x)then
		return math.abs(math.atan((p.y-y)/(p.x-x))) + math.pi
	elseif(p.y <=y and p.x>=x)then
		return math.abs(math.atan((p.y-y)/(p.x-x)))
	elseif(p.y <=y and p.x<=x)then
		return math.pi-math.atan((p.y-y)/(p.x-x))
	elseif(p.y >=y and p.x>=x)then
		return pi2 - math.atan((p.y-y)/(p.x-x))
	end
end
