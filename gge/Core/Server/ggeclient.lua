...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2016-05-27 17:06:12
...
function hpclient:初始化(a,b)
	...
	__gge.print(false,7,"-------------------------------------------------------------------------\n")
	__gge.print(true,7,string.format("创建%s客户端		-->", b or a or ''))
...
function hpclient:连接(ip,port,async)--地址，端口，异步
	...
	__gge.print(false,7,"-------------------------------------------------------------------------\n")
	__gge.print(true,7,string.format('连接"%s:%s"	-->', ip or '127.0.0.1',port))
	if self._hp:Start(ip or '127.0.0.1',port or 9527,async and 1 or 0) ==0 then
...
function hpclient:OnReceive(iLength)
